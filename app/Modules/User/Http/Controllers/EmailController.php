...
class EmailController extends BasicController
{
    ...
    public function activeEmail($validationInfo)
    {
        ...
        $user = UserModel::where('email', $info['email'])->where('validation_code', $info['validationCode'])->where('validation_token',$info['token'])->first();
        ...
        if ($status){
            Auth::login($user);
            //自动发放
            UserModel::sendfreegrant($user->id,1);
